        ...
        $.ajax({
            ...
            success: function(clients){
                ...
                clients.forEach(client => {
                    ...
                    output += "<td>";
                    output += `<button type='button' class='btn btn-info btn-sm' data-toggle='modal' data-target='#exampleModalScrollable' onclick='edit_client_grd("${client[name]}","${client[lastname]}","${client[telephone]}","${client[email]}","${client[idSchool]}","${client[group]}")'>Editar</button>`;
